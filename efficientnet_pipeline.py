            ...
            for f in sorted(os.listdir(cls_dir)):
                if f.endswith('.png'):  # Only PNGs
                    ...
                    if os.path.getsize(path) > 1024:  # Skip empty/corrupted files
                        try:
                            # Test loading the image here to catch issues early
...
class EfficientNetClassifier(pl.LightningModule):
    def __init__(self, class_weights):
        super().__init__()
        self.model = models.efficientnet_b0(pretrained=True)  # Replace with desired EfficientNet variant (e.g., efficientnet_b0)
        num_features = self.model.classifier[1].in_features  # Adjust for EfficientNet's classifier layer structure
        self.model.classifier[1] = nn.Linear(num_features, 2)  # Update for binary classification

        self.loss_fn = nn.CrossEntropyLoss(weight=class_weights)

        # Metric containers and epoch-wise logs (same as DenseNet)
...
# 3. Training Setup (unchanged except model instantiation and checkpoint naming)
# Replace DenseNetClassifier with EfficientNetClassifier in the main function
def main():
    ...
    checkpoint_cb = ModelCheckpoint(
        dirpath="checkpoints_efficientnet",
        filename="efficientnet-{epoch}-{val_loss:.2f}",
        monitor="val_loss",
        mode="min",
        save_top_k=1,
    ...
    model = EfficientNetClassifier(class_weights=weights)  # Use EfficientNetClassifier instead of DenseNetClassifier
    ...
    # Save the model
    torch.save(model.state_dict(), os.path.join(out_model_dir, "uterine_cancer_efficientnet.pth"))
    print("\nTraining Complete! Model saved to out_model/uterine_cancer_efficientnet.pth")
